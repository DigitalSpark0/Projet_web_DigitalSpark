        ...
        <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    ...
                    <h6 class="font-weight-bolder mb-0">Candidatures</h6>
                </nav>
            </div>
        </nav>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th>id_candidature</th>
                                    <th>id_offre</th>
                                    <th>date de candidature</th>
                                    <th>cv du condidat</th>
                                    <th>Disponibilité </th>
                                </tr>
...
if(isset($_POST["type"])) {
    if($_POST["type"] == "tri"){
        $tab = $controller->affichertriCand();
    } elseif($_POST["type"] == "search"){
        $tab = $controller->afficherRechercheCand($_POST["search"]);
    } else {
        $tab = $controller->listCand();
    }
...
foreach ($tab as $candidatures) 
{
    echo "<tr>";
    echo "<td>" . htmlspecialchars($candidatures['id_candidature']) . "</td>";
    echo "<td>" . htmlspecialchars($candidatures['id_offre']) . "</td>";
    echo "<td>" . htmlspecialchars($candidatures['date_candidature']) . "</td>";
    echo "<td>" . htmlspecialchars($candidatures['cv']) . "</td>";
    echo "<td>" . htmlspecialchars($candidatures['disponibilité']) . "</td>";
    echo '<td><a href=deletCandi.php?id_candidature=' . $candidatures['id_candidature'] . '&id_offre=' . $candidatures['id_offre'] . '" name="delete_id">Supprimer</a></td>';
    echo '<td><a href="details_candidature.php?id_candidature=' . $candidatures['id_candidature'] . '" name="details">Détails</a></td>';
    echo "<td><a href='ModifierCand.php?id_candidature=" . $candidatures['id_candidature'] . "' class='btn'>Modifier</a></td>";
...
?>
                        ...
                        <form method="POST" action="pdfCand.php">
        <input type="hidden" name="type" value="pdf">
        <button type="submit" class="">Generate PDF</button>
